  ...
  return (
    <div className="bg-white border-2 border-slate-100 p-8 rounded-[2.5rem] shadow-sm flex flex-col h-full">
      ...
      <div className="mb-6">
        ...
        <p className="text-slate-600 text-lg leading-relaxed italic line-clamp-4">{p.explanation}</p>
      </div>

      <div className="mt-auto">
        <div className="flex justify-between items-center mb-3">
           <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Source Code</h4>
           <button onClick={handleCopy} className="text-xs font-bold text-blue-600 hover:underline cursor-pointer">
             {copied ? "✓ Copied" : "Copy Code"}
           </button>
        </div>
        <pre className="bg-[#0f172a] text-slate-300 p-6 rounded-3xl h-80 overflow-auto custom-scrollbar border border-slate-800 shadow-inner">
          <code className="code-compact whitespace-pre block">{p.code}</code>
        </pre>
      </div>
    </div>
  );
...
export default function AllProblems() {
  ...
  useEffect(() => {
    axios.get(`${API_URL}/problems`).then(res => setProblems(res.data)).catch(err => console.error(err));
  }, []);

  return (
    <div className="container-main py-10 px-4 md:px-10 max-w-7xl mx-auto">
      <h1 className="text-5xl font-black text-slate-900 mb-12 tracking-tighter">Full Archive</h1>
